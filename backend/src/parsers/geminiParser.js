...
**🚨 CRITICAL - 당신이 자주 저지르는 실수들 (절대 하지 마세요!):**

❌ 잘못된 추출 예시 (이렇게 하지 마세요):
{
  "location": "번,건물명칭 및 번호",  ← 이것은 표의 컬럼 헤더입니다!
  "roadAddress": "]",  ← 이것은 마커 기호입니다!
  "buildingName": "및 번호",  ← 이것은 불완전한 텍스트입니다!
  "landRightRatio": "등기원인 및 기타사항",  ← 이것은 다른 컬럼의 헤더입니다!
  "sectionA": [],  ← 데이터가 있는데 비어있으면 안됩니다!
  "sectionB": []   ← 데이터가 있는데 비어있으면 안됩니다!
}

✅ 올바른 추출 예시 (이렇게 해야 합니다):
PDF 텍스트가 이렇게 되어 있다면:
"고유번호 1342-2017-016558
[집합건물] 경기도 광주시 태전동 695 이편한세상태전2차 제102동 제2층 제202호
표시번호  접수  소재지번,건물명칭 및 번호  건물내역
1  2017년11월27일  경기도 광주시 태전동 695  철근콘크리트구조
                  이편한세상태전2차 제102동
[도로명주소]
경기도 광주시 태전동로 12"
...
이렇게 추출해야 합니다:
{
  "uniqueNumber": "1342-2017-016558",  ← "고유번호" 뒤의 값
  "location": "경기도 광주시 태전동 695",  ← 표 안의 주소 부분만
  "roadAddress": "경기도 광주시 태전동로 12",  ← [도로명주소] 다음 줄
  "buildingName": "이편한세상태전2차 제102동 제2층 제202호"  ← 전체 건물명
}

**⚠️ 추출 시 반드시 지켜야 할 원칙:**
1. 표의 **컬럼 헤더**(소재지번,건물명칭 및 번호, 등기원인 및 기타사항 등)는 **절대 데이터가 아닙니다**
2. 마커 기호(], [, -, |, 등)는 **절대 데이터가 아닙니다**
3. 불완전한 텍스트 조각("및 번호", "번,", "사항" 등)은 **절대 데이터가 아닙니다**
4. **갑구와 을구는 반드시 모든 항목을 추출**하세요 (빈 배열로 두지 마세요)
5. JSON만 반환하세요 (마크다운, 설명, 주석 없이)
...
2. 갑구 (【갑구】 섹션 - 소유권에 관한 사항):
   **🚨 CRITICAL: PDF에 갑구 항목이 있다면 반드시 모두 추출하세요! 빈 배열로 두지 마세요!**
   
   **갑구를 찾는 방법:**
   - "【갑구】" 또는 "( 소유권에 관한 사항 )" 텍스트를 찾으세요
   - 그 아래 "순위번호", "등기목적", "접수", "등기원인", "권리자" 등의 컬럼이 있는 표가 있습니다
   - 표의 각 **행(row)**이 하나의 등기 항목입니다
   
   **각 항목에서 추출할 정보:**
   - rankNumber: 순위번호 컬럼 (1, 2, 3 또는 1-1, 3-1 등)
   - purpose: 등기목적 컬럼 (소유권보존, 소유권이전 등)
   - receiptDate: 접수일자 (YYYY-MM-DD 형식으로 변환)
   - receiptNumber: 접수번호 (제XX호)
   - registrationCause: 등기원인 (매매, 증여, 신탁 등)
   - rightHolder: 권리자 이름 ("소유자 " 또는 "권리자 " 뒤의 이름)
   - idNumber: 주민등록번호 (XXX-XXX-******* 형식)
   - address: 주소 전체
   - status: "유효" 또는 "말소" (실선이 그어져 있거나 "말소" 텍스트가 있으면 "말소")
   
   **실제 PDF 예시:**
   만약 PDF에 이렇게 있다면:
   "순위번호  등기목적      접수              등기원인    권리자 및 기타사항
    1       소유권보존    2017년11월27일    신탁        소유자  케이비부동산신탁주식회사
                         제85281호                             110111-1348237
                                                              서울특별시 강남구 테헤란로 124
    3       소유권이전    2018년1월18일     매매        소유자  권지은  800501-*******
                         제5909호                             경기도 광주시 문화로101번길 9..."
   ...
   [
     ...
     },
     {
       ...
       "rightHolder": "권지은",
       "idNumber": "800501-*******",
       "address": "경기도 광주시 문화로101번길 9",
       "status": "유효"
     }
   ]

3. 을구 (【을구】 섹션 - 소유권 이외의 권리에 관한 사항):
   **🚨 CRITICAL: PDF에 을구 항목이 있다면 반드시 모두 추출하세요! 빈 배열로 두지 마세요!**
   
   **을구를 찾는 방법:**
   - "【을구】" 또는 "( 소유권 이외의 권리에 관한 사항 )" 텍스트를 찾으세요
   - 그 아래 "순위번호", "등기목적", "접수", "등기원인", "권리자" 등의 컬럼이 있는 표가 있습니다
   - 표의 각 **행(row)**이 하나의 등기 항목입니다
   
   **각 항목에서 추출할 정보:**
   - rankNumber: 순위번호 (1, 2, 3 또는 9-1, 10-1 등)
   - purpose: 등기목적 (근저당권설정, 전세권설정, 근질권설정 등)
   - receiptDate: 접수일자 (YYYY-MM-DD 형식)
   - receiptNumber: 접수번호 (제XX호)
   - registrationCause: 등기원인 (설정계약, 해지 등)
   - claimAmount: 채권최고액 또는 전세금 (숫자만, 쉼표 제거. 예: "금204,600,000원" → "204600000")
   - debtor: 채무자 또는 전세권설정자 이름
   - rightHolder: 근저당권자, 전세권자, 채권자 이름/회사명
   - status: "유효" 또는 "말소"
   
   **실제 PDF 예시:**
   만약 PDF에 이렇게 있다면:
   "순위번호  등기목적        접수              등기원인    권리자 및 기타사항
    5       근저당권설정    2022년8월12일     설정계약    채권최고액  금144,000,000원
                           제54459호                     채무자  주식회사나무꼴
                                                        근저당권자  주식회사하나은행
    8       근저당권설정    2024년8월22일     설정계약    채권최고액  금204,600,000원
                           제52764호                     채무자  권지은
                                                        근저당권자  주식회사한국스탠다드차타드은행
    10      전세권설정      2025년5월21일     설정계약    전세금  금5,000,000원
                           제2460458호                   전세권자  주식회사에스유대부"
   ...
   [
     ...
     },
     {
       ...
       "receiptNumber": "제2460458호",
       "registrationCause": "설정계약",
       "claimAmount": "5000000",
       "debtor": "",
       "rightHolder": "주식회사에스유대부",
       "status": "유효"
     }
   ]
